...
class Simple {
    ...
    public function isAjaxRequest() {
        ...
        if (isset($this->request->server['HTTP_X_REQUESTED_WITH']) || (!isset($this->request->server['HTTP_X_REQUESTED_WITH']) && $this->request->server['REQUEST_METHOD'] == 'POST')) {
            $ajax = true;
        }
                ...
                if (!empty($rule['enabled'])) {
                    ...
                    switch ($key) {
                        ...
                        case 'api':
                            ...
                            if (!empty($rule['filter']) && isset($this->_fields[$block][$rule['filter']]['value'])) {
                                $filter = $this->_fields[$block][$rule['filter']]['value'];
                                $filterId = $block.'_'.$rule['filter'];
                            }
    ...
    private function getMasterFieldValue($masterId) {
        ...
        if ($this->isFieldUsed($block, $masterId)) {
            ...
            if ($object == 'customer' && $this->_page == 'checkout') {
    ...
    private function isFieldDisplayed($rowSettings) {
        $block = $this->getFieldsBlockName();
        ...
        if (!empty($rowSettings['masterField'])) {
            ...
            if (!empty($masterValue) && !empty($rowSettings['displayWhen'][$masterValue])) {
                return true;
            } else {
                return false;
            }
    ...
    private function isFieldRequired($rowSettings) {
        ...
        if (empty($rowSettings['required'])) {
            ...
            if (!empty($masterValue) && !empty($rowSettings['requireWhen'][$masterValue])) {
                return true;
            } else {
                return false;
            }
    ...
    protected function getFieldDefaultValue($id) {
        ...
        if (!empty($fieldSettings['default']['source']) && $fieldSettings['default']['source'] == 'model' && !empty($fieldSettings['default']['method'])) {
            ...
            $filter = '';
            if (!empty($fieldSettings['default']['filter']) && isset($this->_fields[$block][$fieldSettings['default']['filter']]['value'])) {
                $filter = $this->_fields[$block][$fieldSettings['default']['filter']]['value'];
            }
    ...
    private function getFieldValues($id) {
        ...
        if (!empty($fieldSettings['values']['source']) && $fieldSettings['values']['source'] == 'model' && !empty($fieldSettings['values']['method'])) {
            ...
            $filter = '';
            if (!empty($fieldSettings['values']['filter']) && isset($this->_fields[$block][$fieldSettings['values']['filter']]['value'])) {
                $filter = $this->_fields[$block][$fieldSettings['values']['filter']]['value'];
            }
    ...
    private function getFieldMask($id) {
        ...
        if (!empty($fieldSettings['mask']['source']) && $fieldSettings['mask']['source'] == 'model' && !empty($fieldSettings['mask']['method'])) {
            ...
            $filter = '';
            if (!empty($fieldSettings['mask']['filter']) && isset($this->_fields[$block][$fieldSettings['mask']['filter']]['value'])) {
                $filter = $this->_fields[$block][$fieldSettings['mask']['filter']]['value'];
            }
